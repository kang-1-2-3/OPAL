...
class PcMapLocDataset(data.Dataset):
    ...
    def make_dataset(self, mode = 'train'):
        ...
        kitti_dataset = pykitti.odometry(self.opt['loading']['pc_data_dir'], seq)
        # assert len(kitti_dataset) == len(pc_gps_file)
        if not os.path.exists(os.path.join(f"{self.opt['tiling']['tiles_path']}",f"80_buildingonly_tiles_{seq}.pkl")):
            self.save_seq_tile_manager(seq, pc_gps_file)
        seq_tile_manager = self.load_seq_tile_manager(seq)
        # add data to data_list
        if mode == 'train':
    ...
    def save_seq_tile_manager(self, seq, pc_gps_file):
        ...
        seq_tile_manager.save(os.path.join(self.opt['tiling']['tiles_path'], f"80_buildingonly_tiles_{seq}.pkl"))

    def load_seq_tile_manager(self, seq):
        tile_manager = TileManager.load(Path(os.path.join(self.opt['tiling']['tiles_path'], f"80_buildingonly_tiles_{seq}.pkl")))
        return tile_manager
    ...
    def __getitem__(self, index):
        ...
        # if index % 100 == 0:
            ...
            # plt.axis('equal') 
            # plt.grid(True)
            # plt.savefig(f'{seq}_{seq_index}_pc.png', dpi=300, bbox_inches='tight')
            # plt.close()
        ...
        lidar_data = self.pad_or_trim_to_np(pcs, [81920, 4]).astype('float32')
        ...
        # if index % 500 == 0:
        #     cv2.imwrite(f"datavis/onlybuilding/pc_bev_img_{seq}_{seq_index}.png", pc_bev_img)
        ...
        # if index % 500 == 0:
        #     plt.imsave(f"datavis/onlybuilding/{seq}_{seq_index}_canvas.png", vis.astype(np.float32))
