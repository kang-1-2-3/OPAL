...
from network.network import Pcmapvpr, PcmapvprV2   
...
import pandas as pd
from kitti_dataloader_pp import PcMapLocDataset
...
def validate_model(model: nn.Module, val_loader: DataLoader, criterion: nn.Module, device: torch.device) -> dict:
    ...
    with torch.no_grad():
        ...
        for data in tqdm(val_loader, desc="Validation"):
            xy, osm_data, lidar_data, lidar_mask = data['xy'].to(device), data['osm_map'].to(device), data['lidar_data'].to(device), data['lidar_mask'].to(device) 
            osm_descs, pc_bev_descs = model(osm_data, lidar_data, lidar_mask)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for step, data in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch}/{num_epochs - 1}")):
                ...
                xy, osm_data, lidar_data, lidar_mask = data['xy'].to(device), data['osm_map'].to(device), data['lidar_data'].to(device), data['lidar_mask'].to(device) 
                optimizer.zero_grad()
                osm_descs, pc_bev_descs = model(osm_data, lidar_data, lidar_mask) # [B, 4096] [B, 8192]
                # pad_size = 8192 - 256
            ...
            if if_early_stop:
                ...
                if early_stopping.early_stop:
                    ...
                    if if_send_email:
                        email_body = "Training has been early stopped.\n\n This is KITTI with Circle Loss V2 with lr=10e-6, pc encoder is pointpillar (with semantics) \n\n"
