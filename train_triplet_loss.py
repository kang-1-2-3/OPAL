...
from network.network import PcmapvprV3
...
import pandas as pd
from kitti_dataloader_triplet import PcMapLocDataset
...
def collate_fn(batch):
    ...
    pc_bev_img = [item['pc_bev_img'] for item in batch]
    osm_map = [item['osm_map'] for item in batch]
    xy = [item['xy'] for item in batch]
    neg_osm_maps = [item['neg_osm_maps'] for item in batch]
    neg_xys = [item['neg_xys'] for item in batch]

    pc_bev_img = torch.utils.data.dataloader.default_collate(pc_bev_img)
    osm_map = torch.utils.data.dataloader.default_collate(osm_map)
    xy = torch.utils.data.dataloader.default_collate(xy)

    neg_osm_maps = torch.cat(neg_osm_maps, 0)  
    neg_xys = torch.cat(neg_xys, 0)  

    return {
        'pc_bev_img': pc_bev_img,  # [B, C, H, W]
        'osm_map': osm_map,        # [B, H, W]
        'xy': xy,                  # [B, 2]
        'neg_osm_maps': neg_osm_maps,  # [B * N, 3, 160, 160]
        'neg_xys': neg_xys            # [B * N, 2]
    }
...
def validate_model(model: nn.Module, val_loader: DataLoader, criterion: nn.Module, device: torch.device) -> dict:
    ...
    with torch.no_grad():
        ...
        for data in tqdm(val_loader, desc="Validation"):
            xy, osm_data, pc_bev_data, neg_osm_data = data['xy'].to(device), data['osm_map'].to(device), data['pc_bev_img'].to(device), data['neg_osm_maps'].to(device)
            B, C, H, W = data['pc_bev_img'].shape
            pos_osm_descs, neg_osm_descs, pc_bev_descs = model(osm_data, neg_osm_data, pc_bev_data)
            osm_feture_database.append(pos_osm_descs)
            pc_features.append(pc_bev_descs)
            xy_list.append(xy)
            # loss = criterion(pc_bev_descs, pos_osm_descs, neg_osm_descs)
            loss = 0
            num_negs = neg_osm_data.shape[0]//B
            for i in range(len(pc_bev_data)):
                max_loss = torch.max(criterion(pc_bev_descs[i], pos_osm_descs[i], neg_osm_descs[num_negs*i:num_negs*(i+1)]))
                loss += max_loss
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset, batch_size=config['training']['batch_size'], shuffle=True, drop_last=True, num_workers=config['training']['num_workers'], pin_memory=True, collate_fn=collate_fn)
    print('Train dataset loaded, length:', len(train_loader.dataset))
    val_loader = DataLoader(val_dataset, batch_size=config['training']['batch_size'], shuffle=False, drop_last=False, num_workers=config['training']['num_workers'], pin_memory=True, collate_fn=collate_fn)
